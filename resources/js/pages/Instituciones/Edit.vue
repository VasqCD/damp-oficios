...
function submit() {
    form.put(`/instituciones/${props.institucion.id}`);
}
</script>

<template>
    <Head :title="`Editar ${institucion.nombre}`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-3xl font-bold tracking-tight">Editar Institución</h2>
                    <p class="text-muted-foreground">
                        Modificar datos de {{ institucion.nombre }}
                    </p>
                </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Actualice los datos de la institución
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex justify-end gap-4">
                            ...
                            <Button type="submit" :disabled="form.processing">
                                {{ form.processing ? 'Guardando...' : 'Actualizar Institución' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
