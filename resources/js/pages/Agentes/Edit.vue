...
const form = ref({
    nombres: props.agente.nombres,
    apellidos: props.agente.apellidos,
    cargo_id: props.agente.cargo_id.toString(),
    institucion_id: props.agente.unidad.institucion_id.toString(),
    unidad_id: props.agente.unidad.id.toString(),
    tipo: props.agente.tipo,
    activo: props.agente.activo,
});
...
// Load unidades on mount for the current institution
onMounted(async () => {
    if (form.value.institucion_id) {
        loadingUnidades.value = true;
        try {
            const response = await fetch(`/api/instituciones/${form.value.institucion_id}/unidades`);
...
function submit() {
    ...
    router.put(`/agentes/${props.agente.id}`, form.value, {
        onError: (err) => {
            errors.value = err;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-3xl font-bold tracking-tight">Editar Agente</h2>
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex justify-end gap-4">
                            ...
                            <Button type="submit" :disabled="processing">
                                {{ processing ? 'Actualizando...' : 'Actualizar Agente' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
